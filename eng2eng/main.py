...
if BASELINE:
        encoder = RNNEncoder(inLang.n_words, 256, 256).to(device)
        decoder = RNNDecoder(
            outLang.n_words, 256, 256, inLang.max_length
        ).to(device)
else:
    encoder = Encoder(inLang.n_words, 256, 256).to(
        device
    )
    decoder = Decoder(
        outLang.n_words, 256, 256, inLang.max_length
...
train(dataloader, encoder, decoder, 100, 0.01, 1,1)
